...
PRIMARY_NULL under no current-order effect, PILOT_SUPPORT for a clean positive control,
and CARRYOVER_ARTIFACT when only the previous experimental trial predicts the outcome.
"""
...
def generate(schedule_path: str, scenario: str, seed: int) -> list[dict]:
    ...
    for di, (device, sched) in enumerate(sorted(by_device.items())):
        dev_shift = rng.normal(0.0, 0.03)
        local: list[dict] = []
        for s in sched:
            main = s["trial_class"] == "main"
            run = int(s["run_index"])
            ...
            local.append(row)
        # Make the main AB/BA observable-state means exactly equal within device for
        # the four preregistered equivalence coordinates. This is a software control
        # for the equivalence implementation, not a hardware power claim.
        ...
        for r in local:
            ...
            # Long-washout trials intentionally carry no current-order effect.
            y = 1.0 + z + 0.001 * r["run_index"] + current + previous + rng.normal(0, 0.12)
